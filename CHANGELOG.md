...
### Added
- **Performance regression gate** (`beads-task-issue-tracker-6bm`): vitest bench for `groupIssues` on a deterministic 500-issue fixture (50 epics × 10 children), with a hard 50ms mean threshold (~2× CI runner mean) that catches O(n²) regressions automatically. `pnpm test:bench` runs locally and in CI (ubuntu-22.04, "Performance benchmarks" step in `ci.yml`). `pnpm test` (unit tests) explicitly excludes bench files so they do not run in the normal test pass. README documents manual MCP perf-check workflow for live scenarios (SWR cache, IPC roundtrip, Vue reactivity) that vitest bench cannot observe.
