...
- **Pi plan-mode execute ready-UI no longer hides the plan behind `select`** (`beads-task-issue-tracker-ph4p`): `plan_mode_complete` in TUI opens document-flow `ctx.ui.custom` without `overlay: true` and without `SelectList` (wrap-then-window plan pane first with PgUp/PgDn, four labels at the bottom without descriptions or Preview; hand-rolled 1–4 / arrows after live HA `TUI.stop()`); leftover `agent_settled` stays `select`; `sendMessage(plan-ready-document)` is not treated as visibility proof. Focused vitest under `tests/extensions/plan-mode.test.ts`.
