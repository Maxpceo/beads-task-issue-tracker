...
- **Pi plan widget closes on a chat question and no longer waits for a magic phrase after plan-review** (`beads-task-issue-tracker-1jbs`): a non-confirm chat question while ready-UI is open dismisses the overlay before the answer and sets `discussionOpen`, so the answer turn does not call `plan_mode_complete` or wait on a button. Execute that returns before the answer is visible does not write `PLAN APPROVED` or dispatch a supervisor. Dirty plan-review still does not set `discussionOpen`; the orchestrator calls `record_plan_review_adjudication` to reopen the buttons. Focused vitest under `tests/extensions/plan-mode.test.ts`.
...
- **Pi plan-ready adds fifth «Задать вопрос» action and discussionOpen gate** (`beads-task-issue-tracker-jvti`): ready-UI now includes «Задать вопрос» which sets discussionOpen=true, closes widget without editor, and blocks subsequent plan_mode_complete until Maxim sends a whole-message confirm (trim+lowercase one of: да, ок, ok, покажи план, вопросы закрыты, можно показывать план). plan_mode_complete description updated to note the gate; first complete after plan-mode enable opens widget; discussionOpen cleared on plan-mode off or clear. Orchestrator forbids `## Дальше` after TUI complete. plan-ready-choice legend and exclusive next-action routing preserved. Focused vitest under `tests/extensions/plan-mode.test.ts`.
...
- **Pi `dispatch_supervisor` dryRun is a read-only preview** (`beads-task-issue-tracker-0a2l`): dryRun skips workflow-state/START_COMMIT freshness and allows any non-terminal bead status (including `inreview`/`blocked`) so agent selection can be checked after a bead enters review. Full dispatch without dryRun keeps the previous gates. Headless dryRun no longer writes a DISPATCH comment or emits `workflow-state:update`. Focused vitest under `tests/extensions/beads-dispatch.test.ts` and `tests/extensions/beads-dispatch-cmux.test.ts`.
...
- **Pi interactive inreview next-action points to visible reviewer** (`beads-task-issue-tracker-0lp7`): `[PI INREVIEW GUARD]` and claim guards with `hasUI===true` now lead with `dispatch_reviewer(beadId=<ID>, transport=cmux, cwd=<workflowState.worktreePath>)` and mark `review_bead` as headless-fallback only. Non-UI GUARD/claim keep `review_bead`. Policy, `workflow_submit_for_review`, and `workflow_complete` dual-token texts put `dispatch_reviewer`+`transport=cmux` first with `review_bead`/`headless-fallback`. Tool description and `/review-bead` notify pin the same interactive default. Focused vitest under `tests/extensions/workflow-state.test.ts`, `tests/extensions/beads-policy.test.ts`, and `tests/extensions/review-workflow.test.ts`.

- **Pi plan-mode human transcript before ready/questionnaire UI** (`beads-task-issue-tracker-f3zr`): before blocking ready `select` and questionnaire widgets, the full plan or question prompt+options is written into the chat via `sendMessage({ display: true }, { triggerTurn: false })` (`plan-ready-document` / `plan-questionnaire`). Dirty plan-review findings also go to the transcript (`plan-review-findings`) in addition to the `plan_mode_complete` tool result; clean gate adds a short `plan-review-clean` note. `sendMessage` throws must not block the widget. Leftover `agent_settled` dirty path still uses a single `triggerTurn: true` message. Contract in `.pi/extensions/plan-mode/README.md`; focused vitest under `tests/extensions/plan-mode.test.ts`.

- **Pi merge-slot own-session push after `workflow_complete`** (`beads-task-issue-tracker-9rha`): `requireMergeSlotForPush` no longer blocks same-session `git push` when the own-session merge-slot holder is still held but persisted `sessionKey` was wiped by `workflow_complete`. `effectiveMergeSlotSessionKey` uses a dual path — persisted workflow-state key first, then the current process `id:`-only runtime key — so own-holder push/acquire still pass without `PI_SKIP_POLICY`. Skills document same-session land/merge after complete.
...
- **Pi strict plan ready-UI on `agent_settled`** (`beads-task-issue-tracker-gauq`): blocking `ctx.ui.custom` no longer opens from the `agent_end` lifecycle (that path crashed the TUI session right before the ready actions rendered). Strict complete-when-ready now shows the four-button ready-UI on `agent_settled` only — after Pi has stopped auto-retrying/compacting and it is safe to replace the editor. A TUI/render failure in the ready-UI loop notifies, clears `pendingReadyPlan`, and keeps plan mode ON (strict) instead of killing the session; restored sessions re-open ready-UI on the first `agent_settled` because pending is persisted. Auto/autopilot still skip ready-UI. Contract in `.pi/extensions/plan-mode/README.md`; focused vitest under `tests/extensions/plan-mode.test.ts`.

- **Pi cmux-sidebar landing after close, UNBOUND fallback, RU labels** (`beads-task-issue-tracker-2voh`): after `bd close`, workflow-state keeps `sessionMode`/`state=closed` (instead of idle) so the shared cmux sidebar keeps the blue landing pill (`arrow.up.circle`, progress `0.98`, label «нужен merge») until land/merge clears it; `merged`/`deferred` still clear. Leftover unmapped `sessionMode` such as `UNBOUND_WORKFLOW_STATE` no longer hides a mapped `state` — `resolveModeForVisual` falls back to `state` for the pill. `set-progress --label` uses short Russian stage labels (`взята`…`нужен merge`); `reviewing` uses eyeglasses (distinct from the `inreview` eye). Focused vitest under `tests/extensions/cmux-sidebar.test.ts` and `tests/extensions/workflow-state.test.ts`.

- **Pi Fast Path approve no longer spawns supervisor** (`beads-task-issue-tracker-g1s1`): after `PLAN APPROVED`, a nonempty `FAST_PATH_RATIONALE` field-line with autopilot off skips `dispatch_supervisor` / pre-dispatch continuation. Durable approve still exits plan mode into `sessionMode=implementing` and returns `fastPathSkip`; UI Execute and `/plan-auto` use `triggerTurn: true` so the orchestrator continues in-session, while in-turn `workflow_plan_approved` keeps display-only skip. Without the rationale (or with autopilot on) continuation still spawns the supervisor. Contract in `plan-bead` and `.pi/extensions/plan-mode/README.md`; focused vitest under `tests/extensions/plan-mode.test.ts`.

- **Pi plan-approve main worktree chicken-egg** (`beads-task-issue-tracker-llvr`): claim/reconcile no longer record protected `main`/`master` cwd as task BRANCH/WORKTREE/START_COMMIT; claim-from-main keeps empty task scope until bind. Strict plan mode allows one recovery `bd worktree create <absolute-path> --branch <type>/<basename>` (canonical task branch only; not `git worktree add` / remove/prune; no shell composition). `workflow_plan_approved` recovery embeds executable WORKTREE/BRANCH from plan evidence and retries approve without a second human approval or `workflow_plan_mode off`. Happy path remains claim → worktree create → setup → `workflow_update` → plan mode. Contract in `claim-bead` / `plan-bead` and `.pi/extensions/plan-mode/README.md`.

- **Pi grey-matrix STOP close panes** (`beads-task-issue-tracker-22ed`): when CODE REVIEW is `APPROVED` but acceptance is grey (`FAIL`/`NOT RUN`), `close_visible_dispatch({ beadId, stopClose: true })` closes this bead's live cmux panes on non-terminal `status=reviewed` (not only after `bd close`). `stopClose` tombstones without unlinking isolation/followup (later terminal close cleans leftovers); `pendingFix: true` still skips close and wins over `stopClose`; `in_progress`/`inreview`/`open` + `stopClose` → BLOCKED. Autopilot hop clears autopilot, writes durable `STOP CLOSE:`, and asks Maxim a/b/c without dumping matrix body; NOT APPROVED / missing-evidence keep panes live. Contract in `AGENTS.md`, `review-bead` / `dispatch-supervisor`, and beads-dispatch / plan-mode READMEs.
...
- **Pi cmux-sidebar clear gated on session ownership** (`beads-task-issue-tracker-hm0f`): agent sessions without `activeBead` no longer wipe the shared workspace sidebar. Clear-without-bead runs only when this process previously applied `set` / `set-suffix-only`; terminal modes with a bead still clear unconditionally. Focused vitest coverage under `tests/extensions/cmux-sidebar.test.ts`.
...
- **Pi `review_bead` runtime hash auto-delegate** (`beads-task-issue-tracker-79w0`): when the loaded review-workflow runtime sha256 mismatches the task worktree `.pi/extensions/review-workflow/index.ts` (and that file is in the scoped diff), non-`dryRun` `review_bead` no longer hard-blocks. It writes a durable `REVIEW RUNTIME DELEGATE` comment and spawns a fresh review from `cwd=<worktreePath>` with `PI_REVIEW_RUNTIME_DELEGATED=1` (programmatic worktree module load preferred; fixed `pi --approve` oneshot fallback). The child owns checks → reviewer → matrix → close/`inreview` and must emit `REVIEW RUNTIME: worktree-fresh, sha256=<worktreeSha>` before the reviewer; the parent never `bd close` on the stale path and only succeeds via `bd show` evidence (`closed` + post-DELEGATE worktree-fresh, or `NOT APPROVED` + `inreview` with matching fresh marker). Anti-recursion, 15m timeout, and mid-flight restore-to-`inreview` keep the path fail-closed. Interactive cmux hop pins are unchanged.
...
- **Pi plan-mode complete-when-ready UI** (`beads-task-issue-tracker-3kyi`): strict `/plan` no longer shows Execute after every `agent_end`. Ready-UI opens only after explicit `plan_mode_complete({ plan })`, with document-flow (non-overlay) buttons Исполнить / Остаться / Уточнить / Отправить на plan-review. Questionnaire keeps a single tool name with themed option preview and digit 1–9 hotkeys; RPC/`!hasUI` falls back to capped `select`/`input`. Plan-review from the ready button is critique-only (no `PLAN APPROVED`, no supervisor start, no `workflow_plan_review` cycle increment). Auto/autopilot skip ready-UI. Contract in `.pi/extensions/plan-mode/README.md` and `plan-bead`; focused vitest under `tests/extensions/plan-mode.test.ts`.

- **Pi `workflow_plan_review` cycle cap (max 2)** (`beads-task-issue-tracker-eb4k`): typed `workflow_plan_review` now tracks a plan-mode session cycle counter (max 2 spawns; reset only on plan mode off→on), reserves the slot before spawn so failed spawns still count, and returns exclusive `stopAdvice` (`HARD_BLOCK` / `CONTINUE` / `STOP_SHOW_USER`) with telemetry-only `classifyPlanReviewRisk`. Call 3+ skips spawn and surfaces cached findings with `STOP_SHOW_USER` so the agent shows Maxim the plan instead of looping. Cap is exclusive to the typed tool path; `/plan-auto` `runReviewGateForPlan` is unchanged. Contract in `.pi/extensions/plan-mode/README.md` and `plan-bead` skill.

- **Pi `/agent-models` class membership wizard** (`beads-task-issue-tracker-d2ln`): root menu gains **Состав class** for class-centric `agentClasses` editing — pick class → roster notify (members of that class only, including stale keys) → Добавить / Убрать (inherit) / Перекинуть. Bulk Add stays in the membership loop; confirmed leaf steps write only `agentClasses` (unset does not delete role model overrides); nested Back/Esc without a leaf write leaves JSON unchanged. Menu IA documented in `.pi/agents/README.md`; focused vitest coverage under `tests/extensions/agent-models.test.ts`.
- **Pi `/plan-autopilot` runtime hop (ping → complete → reviewer → close)** (`beads-task-issue-tracker-garn`): while durable `autopilotEnabled` and `plan=off`, plan-mode is the exclusive consumer of inbound visible `[PING]`/`[PING-ERROR]` (`parseVisiblePing` with `taskId=` or `задача <id>`). One `complete_visible_dispatch` per ping (concurrent lock); supervisor `submitted` triggers one `requestReviewerDispatch` (cmux); reviewer `verdict` APPROVED + green matrix runs `finalizeVisibleReviewClose` (simplified → reviewed → accepted → `bd close` → `close_visible_dispatch`) without a second `review_bead` or Maxim «закрывай?». STOP ask with panes live on `[PING-ERROR]`, BLOCKED/NEEDS_CONTEXT, NOT APPROVED, missing bead/worktree, or blocking matrix. `/plan-auto` does not set durable autopilot or consume ping; hop never runs `land` / `merge-to-main`. Skills and `.pi/extensions/plan-mode/README.md` document exclusive-consumer ownership.
- **Pi cmux-sidebar active bead status** (`beads-task-issue-tracker-pomp`, clear ownership `beads-task-issue-tracker-hm0f`): project-local `.pi/extensions/cmux-sidebar` mirrors owned `workflow-state` into the cmux workspace sidebar — status pill (`{bead-suffix} · {title}`), mode progress (claimed→reviewing), and optional description — via `cmux set-status` / `set-progress` / `workspace-action`. Clears on terminal modes (`closed` / `merged` / `deferred`) with a bead, or on empty `activeBead` only when this session previously wrote `set` / `set-suffix-only` (agents that never set do not wipe the shared sidebar); blocked mode uses a red xmark status without progress. Best-effort `bd show` titles, workspace resolve from `CMUX_WORKSPACE_ID` or `cmux identify`, generation-guarded writes, and no-op when no owned snapshot (does not clear foreign leftover). Enabled in `.pi/settings.json`; focused vitest coverage under `tests/extensions/cmux-sidebar.test.ts`.
- **Pi `/plan-autopilot` orchestrator approval path** (`beads-task-issue-tracker-nkiy`): new slash command and NL triggers (`работаю/работать автономно`, `work autonomously`) separate from `/plan-auto`. Shares the multi-agent plan-review gate, records durable `Approved-by: оркестратор` plus `AUTOPILOT: true`, keeps a session `autopilot` flag after `plan=off`, and documents close-without-«закрывай?» stop conditions (NOT APPROVED / matrix FAIL/NOT RUN/BLOCKED/SCOPE GAP). Does not run `land` / `merge-to-main`. Typed tool: `workflow_plan_mode(mode=autopilot)`. Contract in `.pi/extensions/plan-mode/README.md`.
- **Pi `/agent-models` searchable compact model picker** (`beads-task-issue-tracker-khec`): TUI `ctx.ui.custom` model step is a searchable `Input` + `SelectList` (viewport 12, «Фильтр моделей: ») with case-insensitive substring filter. RPC/missing custom falls back to optional filter input then `select`; catalogs ≥ 40 models cap at 30 + notify. Handler forwards `ctx.mode` and `ctx.ui.custom`.
- **Pi pi-extension skill checklist** (`beads-task-issue-tracker-fkad`): added a thin project-local `.pi/skills/pi-extension` skill for authoring `.pi/extensions` TUI work — `ctx.ui.custom` factory rules, SelectList/Input product filters, MenuUi forwarding traps, and `tests/mocks/pi-tui.ts` vitest gaps — without copying Pi package docs into the repo.
- **Pi `/agent-models` menu, thinking, and discovery** (`beads-task-issue-tracker-bw0x`): bare `/agent-models` opens an interactive menu (model/class/thinking pickers) when UI is available and falls back to a text `show` dump headless. Agent list is discovered from project `.pi/agents/*.md` unioned with existing JSON keys (new agents appear unmapped → session inherit; stale keys can be removed). Independent thinking resolve is `roles[agent].thinking` → `classThinking[class]` → inherit (omit `--thinking`); explicit `off` is not inherit and passes `--thinking off`. Role set/unset merges model and thinking fields. Typed spawn paths (`dispatch_supervisor` / `dispatch_reviewer` headless+cmux, `review_bead`, `subagent` / `plan_subagent`) append resolved `--thinking`. Secondary CLI adds `set|unset class-thinking` and `set|unset role-thinking`. Contract documented in `.pi/agents/README.md`.
- **Pi per-agent model routing** (`beads-task-issue-tracker-sota`): project-local `.pi/agent-models.json` maps agents to power classes (`strong` / `standard` / `cheap`) or per-role model overrides. Shared resolve order is `roles[agent].model` → `classes[agentClasses[agent]]` → session inherit (no `--model`). Typed spawn paths (`dispatch_supervisor` / `dispatch_reviewer` headless+cmux, `review_bead`, `subagent` / `plan_subagent`) pass the resolved model; missing/invalid config inherits the session model without failing spawn. `/agent-models` show/set/unset edits only the project file (not `~/.pi`). Role matrix and fallback are documented in `.pi/agents/README.md`.
...
- **Live supervisor pane reuse** (`beads-task-issue-tracker-kp4r`): typed `followup_visible_dispatch` hops follow-up task text into a waiting visible supervisor pane, respawns shell/dead panes without first-spawn readiness, and blocks hung/busy duplicates instead of starting a second process.
- **Visible code-reviewer cmux pane** (`beads-task-issue-tracker-esg3`): interactive `dispatch_reviewer` accepts `transport: "cmux"` to spawn one role-scoped reviewer pane beside a live supervisor. After the reviewer `[PING]`, `complete_visible_dispatch` records the `CODE REVIEW` verdict without `submit-for-review`. Interactive `review-bead` hops through ping/complete instead of headless `review_bead`; omit `transport` for the headless fallback. Missing cmux stays BLOCKED, not a silent fallback.
- **Visible cmux supervisor dispatch** (`beads-task-issue-tracker-t65n`): interactive `dispatch_supervisor` can spawn a live cmux pane (`transport: "cmux"`). Spawn-ack is not completion. After the supervisor `[PING]`, `complete_visible_dispatch` writes `DISPATCH RESULT` and submits for review only when the supervisor artifact is complete. Plan-approval continuation passes `transport: "cmux"` and waits for ping. Explicit `transport: "headless"` remains the dark-window path; missing cmux in interactive is BLOCKED, not a silent fallback.
...
- **Pi workflow-chain dashboard extension** (`beads-task-issue-tracker-r62v`): project-local `.pi/extensions/workflow-chain` adds `/workflow-chain` (`/chain`) commands for listing configured chains, dashboard-style dry-runs, and safe read-only/demo execution while keeping workflow-critical typed steps as explicit handoffs.
...
- **Pi subagent team dashboard UX** (`beads-task-issue-tracker-1ekq`): the project-local `.pi/extensions/subagent` status panel now shows dashboard-ready cards for planned reviewer/supervisor teams, clearer availability summaries, and copyable dispatch snippets while keeping unavailable-agent guidance non-blocking.
- **Pi subagent persistent agent dashboard** (`beads-task-issue-tracker-wqbb`): `/agents-dashboard` now renders a persistent project-local agent grid with idle/running/completed/error cards, named-team selection from `.pi/agents/teams.yaml`, refresh/clear commands, and live subagent progress updates without dispatching workflow-critical actions.
- **Pi agents dashboard active mode** (`beads-task-issue-tracker-hft1`): `/agents-dashboard active` now shows only agents observed in the current Pi session, `/agents-dashboard all` preserves the full idle-agent grid, and typed workflow dispatch/review agents publish running and terminal cards to the same dashboard.
...
- **Pi Warp notification extension** (`beads-task-issue-tracker-tmfe`): removed project-local `.pi/extensions/warp-notifications`, its `/warp-notify-settings` and `/warp-notify-test` commands, and the matching runtime harness coverage.

### Changed

- **Pi-first workflow unbind from Claude Code** (`beads-task-issue-tracker-4a9g`, `beads-task-issue-tracker-6vrq`): active agent source of truth is `AGENTS.md` + `.pi/*` only. `path-rules` loads nested `AGENTS.md` / `PI_RULES.md` (no longer `CLAUDE.md`); backend nested rules live in `src-tauri/PI_RULES.md`. Pi agents/skills/rules drop always-on Claude path-exclusion guidance. Public contributor docs (`README.md`, `docs/releasing.md`) point to Pi skills/release and `src-tauri/PI_RULES.md` for Tauri MCP; `.claude/` is not deleted and is no longer advertised as the active workflow.
- **Visible cmux right-half capacity 4–6 agents** (`beads-task-issue-tracker-k9j6`): layout contract no longer treats N>2 as unguaranteed. Orchestrator stays exclusive ~50% left; agents pack side-by-side on the right half via existing `resolveVisibleSplitAnchor` (no hard N=2 in code). Dual remains HA-proven (`f8fl`); Maxim accepts practical **4–6** simultaneous agent panes; beyond ~6 is operator judgment. Docs: `AGENTS.md`, dispatch-supervisor skill, cmux-transport JSDoc.
- **Pi `/agent-models` UX polish** (`beads-task-issue-tracker-ym9f`): interactive menu gains nested Back/Esc navigation, Russian-first IA (compact overview, class wizard, agent wizard with badges, cleanup with bulk stale-key confirm), live model catalog from `ctx.scopedModels` / `modelRegistry.getAvailable()` with timeout/empty fallback to config models plus free-text, and Pi-canon `supportedThinkingLevels` filtering (`reasoning === false` → `off` only; map/`xhigh`/`max` rules; warn + reset/repick when stored thinking is unsupported after a model change). Unconfirmed mid-step choices do not write JSON; confirmed leaf steps save immediately. Spawn stays pass-through for stored levels; CLI still accepts the full thinking enum without registry filter. Contract documented in `.pi/agents/README.md`.
...
- **Pi workflow guard localization** (`beads-task-issue-tracker-snnb`): Pi workflow-state, dispatch, review, and policy guards now report blocking and recovery guidance in Russian while preserving technical identifiers, command names, and status labels.
...
- **Honor `prefers-reduced-motion` for JS-driven smooth scroll** (`beads-task-issue-tracker-owo6`): `scrollBehavior()` utility in `app/utils/motion.ts` collapses `scrollIntoView` calls to `behavior: 'auto'` when the OS reduced-motion preference is active. Applied to `useKeyboardNavigation`, `MarkdownPreviewDialog`, `CommentSection`, and `IssueTable`. Loading spinners (`animate-spin`, `animate-pulse`, `animate-ping`) are explicitly exempt from the global CSS reduced-motion reset — a frozen spinner reads as "app hung"; WCAG 2.3.3 explicitly permits progress indicators to keep animating.
...
- **Pi plan approval task worktree scope** (`beads-task-issue-tracker-typu`): `workflow_plan_approved` now preserves the recorded task worktree/branch/start scope when approval is recorded from a main checkout, validates explicit approved-plan `Worktree`, `Branch`, and `START_COMMIT` evidence when present, and blocks invalid or mismatched scope before writing bd comments or session state.
- **Pi plan approval durability** (`beads-task-issue-tracker-z098`): approving a plan from the `/plan` UI now writes the durable `PLAN APPROVED` bd comment before switching workflow state to implementation, normalizes missing approval sections for dispatch readiness, and stays blocked with recovery guidance if the comment write fails.
...
- **Pi remote cleanup policy after returning to main** (`beads-task-issue-tracker-8trr`): `beads-policy` now still permits the documented merge-to-main fallback remote `task/...` branch cleanup after the active workflow branch has returned to `main`, while keeping lease, merge-slot, ancestry, protected-branch, and actionable denial checks in place.
- **Pi canonical branch cleanup fallback** (`beads-task-issue-tracker-3eyi`): merge-to-main remote branch cleanup fallback now accepts all canonical Pi branch prefixes (`feat`, `fix`, `docs`, `refactor`, `test`, `chore`, `ci`, and `task`) instead of only `task/...`, while preserving merge-slot, lease, ancestry, and protected-branch safety checks.
...
- **Pi workflow-state plan approval recovery** (`beads-task-issue-tracker-s913`): plan approval now preserves active bead/worktree scope, rejects unsafe same-session recovery with foreign branch/worktree evidence, and clears impossible approved/implementing states when no safe active bead can be recovered.
- **Pi workflow claim runtime recovery** (`beads-task-issue-tracker-h9lm`, `beads-task-issue-tracker-8ah8`, `beads-task-issue-tracker-vyjn`, `beads-task-issue-tracker-5mut`, runtime acceptance `beads-task-issue-tracker-p82u`): typed `workflow_claim` now reports actionable failure details, recovers open already-assigned beads by forcing `in_progress` when needed, preserves current-session ownership evidence for already-`in_progress` claims, supports plan-mode claim API registration through Pi proxy objects, and keeps stale workflow-state update contexts non-fatal after session replacement.
...
- **Epic grouping preserved when epic is filtered but children are visible** (`beads-task-issue-tracker-mczr`): `groupIssues` now includes a pagination safety net — when pagination separates an epic from its visible children, the epic wrapper is restored at the position of the first visible child. Prevents children from rendering as standalone rows without their epic context.
- **Drop noisy Vue cold-start render-race from `beads.log`** (`beads-task-issue-tracker-d3o`): in dev mode, Tauri's webview cold start sometimes loses an async chunk request to a custom-IPC-protocol → postMessage fallback race, which causes Nuxt to render `<NuxtErrorPage>` and Vue 3.5.27 to log `null is not an object (evaluating 'currentRenderingInstance.ce')` from `renderSlot`. The app self-recovers (purely cosmetic, never reproduces in production builds), but the multi-line stack used to flood `beads.log`. `app/plugins/console-to-log.client.ts` now drops messages matching this exact signature (`currentRenderingInstance.ce` + `renderSlot`) before forwarding to the file logger; the original `console.error` still fires, so DevTools output is unchanged.
- **Russian plural forms now correctly follow CLDR rules in all UI counters** (`beads-task-issue-tracker-tei`): added a CLDR-compliant `pluralRule` for Russian (`one`/`few`/`many`, supporting 3- and 4-form messages). Fixes wrong noun forms in "Выбрано N задач", child-issue summary, attachment counter, and bulk-delete confirmation. `i18n/i18n.config.ts` moved from the project root into `i18n/` and wired up via `nuxt.config.ts`.
- **Sync: external bd CLI updates (e.g. `bd update --claim` / `bd close` from Claude Code) now refresh the UI within ~1s instead of requiring a manual refresh.** Multiple compounding causes: (1) `bd_poll_data` recorded the post-command mtime, absorbing external writes into the cache key; fixed by using the entry-time mtime. (2) Watcher fires on `.dolt/*` writes that precede the 5s auto-flush of `issues.jsonl`, but `bd_check_changed` reads only jsonl mtime and would abort the poll; fixed by skipping the redundant mtime gate when the trigger came from the watcher. (3) `POLL_MEMO` could still serve stale data because its key (jsonl mtime) hadn't moved yet; fixed by invalidating the memo for the affected project on each watcher emit. Watcher events now also use `requestImmediatePoll` to bypass the 2s scheduler backpressure window; self-write cooldown still gates first so the sh7 regression is preserved. (`beads-task-issue-tracker-ek33`)
- **Hook `block-worktree-in-repo.sh` flag-whitelist: `--lock` is boolean, `--orphan` takes a value** (`beads-task-issue-tracker-ega`): `extract_worktree_path` previously listed `--lock` as a value-taking flag (false-positive deny on `git worktree add --lock <path>`) and missed `--orphan` (false-positive deny on `git worktree add --orphan <branch> <path>`). Removed `--lock` from the skip-next whitelist (boolean flags fall through `-*) continue ;;`) and added `--orphan`. Both were safe-failures (over-block, not under-block). +3 unit tests.
- **Replace `transition: all` with explicit property list in `.search-slide` animation** (`tailwind.css`): replaced the catch-all `transition: all 0.15s ease-out` with five explicit properties (opacity, max-height, margin-bottom, padding-top, padding-bottom) to avoid unexpected reflows on unrelated property changes and follow the WIG "list properties explicitly" rule.
- **Replace all 14 `transition-all` Tailwind utilities with explicit alternatives** (`beads-task-issue-tracker-wf5g`): buttons get `transition-colors` (button primitive) or `transition` shorthand (hover/active buttons in IssuePreview, IssueForm, ImageThumbnail); progress bars get `transition-[width]` (StatusChart, IssueTable) or `transition-[width,background-color,box-shadow]` (PriorityChart); sidebars get `transition-[width,opacity]`; detach icon gets `transition-opacity`. Reduces unnecessary property animation and avoids GPU repaints on layout-unrelated properties.
...
- **bd identifiers no longer translated in the create/edit dialog and filter dropdowns** (`beads-task-issue-tracker-3tn`): `issues.typeLabels`, `issues.statusLabels`, `issues.priorityLabels` in `i18n/locales/ru.json` now mirror the English labels (Bug/Task/Feature/Epic/Chore; Open/In Progress/Blocked/Closed/Deferred/Pinned/Hooked; P0 - Critical … P4 - Minimal). Previously ru locale showed a mix of Russian translations (Баг/Задача/Фича/Эпик/Рутина; Открытая/В работе/…; P3 — Низкий) and untranslated English values (Spike/Story/Milestone/Hooked), breaking the `.claude/rules/locale-sync.md` rule that bd identifiers (status/type/priority/labels) are not translated. Badge components in the table and notification strings are unaffected.
- **Global prefers-reduced-motion support** (`beads-task-issue-tracker-etd`): added system-level reduced-motion CSS helper that collapses all animation/transition durations when the OS-level "Reduce motion" preference is on (macOS Accessibility, Windows Ease of Access). NotificationToast keeps a short fade override so status messages remain visible. No new app setting — respects the OS preference.
- **NotificationToast a11y: touch target, focus-visible ring, decorative icon aria-hidden** (`beads-task-issue-tracker-w3f`): dismiss button now has `p-1` padding (24×24px, WCAG 2.5.8 AA), canonical `focus-visible:ring-[3px]` ring matching other UI primitives, and both the type icon and XIcon carry `aria-hidden="true"` to suppress duplicate announcements.
- **NotificationToast accessibility fixes** (`beads-task-issue-tracker-gr6`): added `role="status" aria-live="polite" aria-atomic="false"` to the static toast container so screen readers announce new notifications; added `aria-label` (i18n) to the dismiss button; replaced `transition: all` with explicit `opacity, transform` to avoid compositor anti-pattern.
- **Backend logging flag no longer resets on webview reload** (`beads-task-issue-tracker-7xr`): DebugPanel mount watcher no longer clobbers `LOGGING_ENABLED` — removed `{ immediate: true }` from the `isOpen` watcher so `setLoggingEnabled(false)` is not called on every mount/reload when the panel is closed. [bd-7xr]
- **Perf probe `[perf:bd_version_probe]` now always emits to beads.log, bypassing the `LOGGING_ENABLED` user toggle.** Previously the line was lost on cold start because `get_cli_client_info()` fires before the frontend enables logging. (`beads-task-issue-tracker-8ll`)
...
- **Session-created beads surfaced in session-end reports** (`beads-task-issue-tracker-hp0`): `merge-to-main` Step 7 report and `land` final summary now carry a dedicated `Follow-up beads` row plus a `## Bead'ы, созданные в этой сессии` section listing every `bd create` made during the session (pre-existing findings from RAMS/WIG/detective review, edge-cases, out-of-scope follow-ups). Simple template-only change — relies on orchestrator discipline, with a `bd list --json | jq '.[] | select(.created >= …)'` fallback for validation. Automated hook-based tracking is deferred until this path proves insufficient.
...
- **Remove dead `isSearchActive` prop binding** in `app/pages/index.vue`: the computed `isSearchActive` (line 715) and its two `:is-search-active` bindings on `IssueListPanel` (lines 983, 1208) were inherited from an upstream cherry-pick but never consumed anywhere — `IssueListPanel.vue` did not declare it in `defineProps`, and no child component read it. Surfaced as a Vue warning (_"Extraneous non-props attributes (is-search-active) were passed to component but could not be automatically inherited because component renders fragment or text or teleport root nodes"_) that became more visible after the Nuxt 4.4.2 upgrade. Deleted both bindings and the computed
