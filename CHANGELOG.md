...
- **Pi cmux-sidebar active bead status** (`beads-task-issue-tracker-pomp`): project-local `.pi/extensions/cmux-sidebar` mirrors owned `workflow-state` into the cmux workspace sidebar — status pill (`{bead-suffix} · {title}`), mode progress (claimed→reviewing), and optional description — via `cmux set-status` / `set-progress` / `workspace-action`. Clears on terminal modes (`closed` / `merged` / `deferred`) or empty active bead; blocked mode uses a red xmark status without progress. Best-effort `bd show` titles, workspace resolve from `CMUX_WORKSPACE_ID` or `cmux identify`, generation-guarded writes, and no-op when no owned snapshot (does not clear foreign leftover). Enabled in `.pi/settings.json`; focused vitest coverage under `tests/extensions/cmux-sidebar.test.ts`.
