...
### Fixed
- **Вал toast'ов «Задача удалена» при переключении проектов** (`beads-task-issue-tracker-dp9`): устранены пять источников ложных уведомлений при project switch. Реальный корневой источник (найден через live-инструментацию через Tauri MCP + chrome-devtools MCP): `watch` на persisted-per-project `filters` в `index.vue` — при смене проекта реактивные фильтры перезагружаются из `useProjectStorage`, что триггерит `fetchIssues()` без защиты; diff между оставшимися в `issues.value` задачами проекта A и свежими задачами проекта B трактуется как массовое удаление. Плюс guard `isProjectSwitch` был асимметричным — `addedIds > oldLen × 0.5` не срабатывал при big→small переходе (996 → 131, 131 < 498). Фиксы: (1) `index.vue` — `fetchIssues(false, false, { skipNotifications: true })` в watch-обработчике фильтров; изменение фильтров никогда не означает удаление задачи. (2) `useIssues.ts` — симметричный guard `commonCount < BOTH.length × 0.5` покрывает swap и big↔small. (3) `usePollScheduler` получил `stop()`/`resume()` — inflight poll после stop не обновляет `lastPollEnd` и не вызывает `pollFn`. (4) `useChangeDetection.stop()` ставит `abandoned=true` — inflight `onChanged` не попадает в UI. (5) `handlePathChange` вызывает `stopScheduler()/stopPolling()/await stopListening()` до первого async, и `resumeScheduler()` до `startListening()` (иначе первое watcher-событие после switch дропалось). `fetchPollData` / `fetchIssues` получили опцию `{ skipNotifications: true }` для handlePathChange как дополнительная страховка. Верифицировано воспроизведением через MCP: до фикса — `FIRED deleted=996`, после — `isProjectSwitch=true` на всех diff'ах, 0 toast'ов «удалена». 334/334 тестов зелёные.
...
### Changed
- **CLAUDE.md сокращён со 174 до 142 строк** (child #3 эпика `a4q`): секции «Bead Status Lifecycle», «Merge-slot для параллельных сессий», «Session Completion», «Before Merge to main» сжаты до ссылок на skills/references; добавлена секция «Workflow Skills» (5 пунктов). Архитектурный лимит ≤200 строк соблюдён с запасом. **`.claude/rules/frontend-reviews.md` переписан**: supervisor больше не запускает RAMS/WIG (plugin skills не наследуются subagent'ами) — это orchestrator-step через skill `reviewing-code` Step 2.5. **`.claude/agents/vue-supervisor.md`** обновлён под новый workflow (RAMS/WIG-блок заменён на ссылку). `.claude/rules/locale-sync.md`, `.claude/references/review-chain.md`, `.claude/references/bd-commands.md`, `.claude/beads-workflow-injection.md` — добавлены ссылки на новые skills. `.claude/agents/{tauri,test}-supervisor.md` — комментарий о том, что commit+push последовательность это inline-версия `land`.

### Added
- **`block-git-add-all` hook** (`beads-task-issue-tracker-aas`, child #4 эпика `a4q`): портирован из `invest_fund_sharks`. Блокирует `git add -A`, `git add .`, `git add --all` и `git commit -a` — заставляет перечислять файлы по именам. Token-aware матчинг через `lib/shell-tokens.sh`: `bd add <id>` или цитированные строки внутри `bd comments add ... "git add ."` не триггерят false-positive. Вставлен вторым в `PreToolUse.matcher=Bash` (после `enforce-worktree-fresh-vs-main.sh`).

- **Bead enrichment hook + workflow templates** (`beads-task-issue-tracker-402`, child #2 эпика `a4q`): портирован `enforce-bead-enrichment.sh` с двумя ветками — Bash-matcher блокирует `bd create` без `### Files` / `### Current state` / `### Target state` markers (exempt: `--type=epic`, `--ephemeral`, batch-режимы); Task-matcher блокирует dispatch на `*-supervisor` без enrich + PLAN-comment (exempt: `merge-supervisor`). Grandfathering по `INSTALL_DATE=2026-04-20`. Override: `SKIP_ENRICH_CHECK=1`. Адаптация под текущую версию bd: `.comments` отсутствует в `bd show --json`, поэтому используется `bd comments {ID}` для извлечения. Reference `.claude/references/workflow-templates.md` (heredoc-шаблон, PLAN-комментарий, dispatch skeleton, phase-boundary). `CLAUDE.md` секция «Enrich bead with context» сжата до ссылки на reference + описания hook-поведения.

- **Stale worktree guard + helpers** (`beads-task-issue-tracker-4ss`, child #1 эпика `a4q`): портированы из `invest_fund_sharks`. Новый PreToolUse:Bash хук `enforce-worktree-fresh-vs-main.sh` блокирует `git commit/cherry-pick/rebase/revert/merge` в worktree, отставшем от `origin/main`, когда staged-файлы пересекаются с изменениями в main (риск франкенштейн-коммита). Soft-reminder при отставании без пересечения. Escape: `CLAUDE_SKIP_STALE_CHECK=1`. Helpers `lib/shell-tokens.sh` (token-aware command matcher) и `lib/subagent-detect.sh` (CWD/transcript/marker-lock детект subagent). Reference `.claude/references/bd-worktrees.md` (когда оправдан, команды, ручной E2E). `.gitignore` дополнен `.worktrees/`. `.claude/settings.json` — хук вставлен первым в `PreToolUse.matcher=Bash`.

- **SWR warm-up диск-кэша при cold start приложения** (`beads-task-issue-tracker-t8x`): паттерн stale-while-revalidate, ранее работавший только при переключении проектов (`handlePathChange`), теперь применяется и в `onMounted` — первый запуск приложения. Раньше cold-open показывал loading-state ~2.4с пока шёл cold `bd list`/`bd ready` subprocess (см. цифры h90: spawn 1678ms + 717ms на Invest Sharks 1001 issues). Теперь `app/pages/index.vue:onMounted` перед `fetchIssues()` вызывает `warmUpFromCache(beadsPath.value)` → `applyWarmUpSnapshot` → `updateFromPollData(issues.value, cachedReady)` (тот же паттерн что в `handlePathChange:502-523`). Если на диске есть свежий PollData snapshot (`<os-cache>/com.beads.manager/poll/poll-<hash>.json`, TTL 1 час) — UI рендерится мгновенно (~50мс ipc), а параллельно `fetchIssues+fetchStats` тихо подтягивают свежие данные. Generation guard в `onMounted` не нужен (нет concurrent caller'а как в switch). Лог `[perf:app_boot]` расширен полями `warmup=Xms warmedFromCache=true|false`. Trade-off: первую секунду пользователь может увидеть слегка устаревшее состояние (например, задачу закрытую через `bd close` в терминале — она моргнёт перед обновлением). Это приемлемо — индустриальный паттерн SWR (Linear, Gmail, Vercel SWR библиотека). Связано с `ydr` (batched `fetchPollData` в `onMounted`) — отдельная беда. 326/326 тестов зелёные, `vue-tsc --noEmit` exit 0.

### Internal
- **Performance-инструментация project switch + poll** (`beads-task-issue-tracker-8ug`): для диагностики 5-секундных задержек на проекте из ~1000 issues добавлены `performance.now()` / `Instant::now()` метки в ключевых точках. Backend (`src-tauri/src/lib.rs:bd_poll_data`): разделены фазы `spawn_ms` (subprocess bd), `parse_ms` (JSON parse), `transform_ms` (цикл `transform_issue`), `total_ms`; лог `[perf:bd_poll_data]` уровня `debug` (высокочастотный polling). Отдельный лог `[perf:dolt_coldstart] path=... init_ms=...` уровня `info` срабатывает один раз на проект за процесс через `DOLT_COLDSTART_LOGGED: LazyLock<Mutex<HashSet<String>>>` (паттерн идентичен `LAST_KNOWN_MTIME`). Frontend (`app/composables/useIssues.ts`): `[perf:fetchPollData] ipc=... process=... total=... count=...` уровня `debug`; `[perf:warmUpFromCache] ipc=... process=... total=... count=... hit=true|false` уровня `info` (редкое событие project switch). `app/pages/index.vue:handlePathChange` — добавлены метки `perfWarmup`, `perfClearState`, расширен итоговый лог `[perf:handlePathChange] warmup=... clearState=... pre-flight=... fetchIssues=... fetchStats=... total=... warmedFromCache=...`. Без production-эффекта — чистая диагностика для последующего `beads-task-issue-tracker-h90`. 326/326 тестов зелёные, `vue-tsc --noEmit` exit 0, `cargo check` exit 0.

### Added
- **Stale-while-revalidate disk-кэш PollData** (`beads-task-issue-tracker-9id`): при смене проекта экран больше не пустеет на несколько секунд. После успешного `bd_poll_data` backend фоново пишет сырой снапшот `Vec<BdRawIssue>` (open + closed + ready) в `<os-cache>/com.beads.manager/poll/poll-<hash>.json` через `std::thread::spawn` (fire-and-forget). Хэш проекта — тот же djb2-алгоритм из `app/utils/hash.ts`; Rust-реализация `hash_path_djb2` использует `encode_utf16()` для паритета с JS `charCodeAt` на non-ASCII путях (сверено на 5 путях, включая кириллицу `/Users/максим/project` и surrogate-pair emoji `/tmp/🚀/repo`). TTL 1 час — старше игнорируется. Новая Tauri-команда `bd_poll_data_cached(cwd)` читает кэш и прогоняет через `transform_issue`, возвращая `Option<PollData>`. Frontend: `bdPollDataCached()` в `app/utils/bd-api.ts` + чистая `warmUpFromCache(cwd)` в `useIssues.ts` (возвращает snapshot без мутации реактивного state) + синхронная `applyWarmUpSnapshot`. `handlePathChange` в `app/pages/index.vue` после `await warmUpFromCache` проверяет generation guard **до** мутации state (закрывает race при быстром свитче A→B→C), если snapshot есть — пропускаем `clearIssues()`/`clearStats()` и сразу показываем последний срез; параллельно `fetchPollData()` тихо заменит свежими. Вынесена общая `linkParentsAndChildren(issues)` в `app/utils/issue-helpers.ts` — устранила ~120 строк дубликата между `fetchIssues`/`fetchPollData`/`warmUpFromCache`. 326/326 тестов зелёные (+5 parity-кейсов для `hashPath`, +3 для `linkParentsAndChildren`), `vue-tsc --noEmit` exit 0, `cargo check` exit 0.
- **KPI Deferred + In Review на дашборде + Open → Ready** (`beads-task-issue-tracker-6pg`): пользователь заметил, что отложенные (`deferred`) задачи нигде не видны в быстрой статистике. Ресёрч Linear / Jira / Asana / Height / ClickUp показал: все крупные трекеры выделяют Waiting/On Hold и In Review отдельными метриками. Добавлены 2 новые карточки **In Review** (сводный счётчик review-chain: `inreview`+`simplified`+`reviewed`+`accepted`, отдельный фиолетовый токен `--color-status-inreview` — `#a855f7` dark / `#7c3aed` light) и **Deferred** (`status==='deferred'`, янтарный `--color-status-deferred`); клики активируют соответствующие фильтры в таблице. Дополнительно переименована карточка **Open → Ready** — метрика `stats.open` уже считает только `open && !blockedBy`, поэтому «Open» вводило в заблуждение. Итого 7 KPI-карточек в порядке Workflow → Ready → In Progress → In Review → Blocked → Deferred → All, помещаются в сайдбаре через `flex-wrap` в 2 ряда. `DashboardStats` расширен полями `inReview: number` и `deferred: number` (`app/types/issue.ts`); `computeStatsFromIssues` (`app/utils/issue-helpers.ts`) инкрементирует их после проверки `isIssueBlocked`. `KpiFilter` в `DashboardContent.vue` и `pages/index.vue` синхронно расширен; `activeKpiFilter` распознаёт Set ревью-статусов через `isStatusSetEqual`. i18n: `dashboard.kpi.{ready,inReview,deferred}` добавлены в `en.json` + `ru.json` (EN: Ready/In Review/Deferred, RU: Готовы/На ревью/Отложены); ключ `kpi.open` заменён на `kpi.ready`. 318/318 тестов зелёные, `vue-tsc --noEmit` exit 0.

### Internal
- **Правило `locale-sync` для агентов** (`beads-task-issue-tracker-2e1`): формализовано правило i18n по паттерну `logging` — тройная доставка в rules-архитектуре. L3 rule `.claude/rules/locale-sync.md` с `paths: app/**/*.{vue,ts}` + `i18n/locales/*.json` (источник правды: `$t()`-only в UI, naming-convention с существующими namespace'ами `about`/`app`/`common`/`dashboard`/`details`/`issues`/`layout`/`menu`/`notifications`/`page`/`settings`, обязательная sync en↔ru через `jq -S 'paths(scalars)' | diff`, перечень того что НЕ переводится — пользовательский контент / bd-идентификаторы / имена файлов / native macOS menu). L1 строка в `CLAUDE.md` рядом с Logging. Пункт в `.claude/agents/vue-supervisor.md` в секции Standards — чтобы правило гарантированно приходило к subagent при dispatch (root CLAUDE.md в subagent не инжектится автоматически, а rules с `paths:` не срабатывают на Write нового файла). Отклонены варианты: отдельный i18n-specialist subagent (overkill для 2 локалей, у нас нет отдельной переводческой роли); L5 hook (educational value = 0, блокировка без объяснения); nested `app/CLAUDE.md` (для одного правила — рано).

### Changed
- **KPI-лейблы возвращены на английский + metric-explainer tooltips** (`beads-task-issue-tracker-5n8`): визуальная проверка предыдущего коммита обнажила проблему сетки — русские лейблы («ЗАБЛОКИРОВАННЫЕ») шире английских и нарушают выравнивание карточек. Ресёрч dev-first трекеров (Linear, Height, Vercel, Sentry, Arc, Raycast) показал: все они оставляют статусы на английском. Русские значения `dashboard.kpi.*` в `ru.json` возвращены на английский (Workflow/Ready/In Progress/In Review/Blocked/Deferred/All) — matching status-badges таблицы, которые уже рендерятся через `useStatuses.ts`. Добавлен namespace `dashboard.kpi.tooltip.*` (7 ключей, EN + RU) с техническими пояснениями метрик (паттерн Vercel/Stripe/Mixpanel — metric-explainer tooltip). `KpiCard.vue` расширен опциональным prop `tooltip?: string`: когда передан, кнопка оборачивается в shadcn `<Tooltip>`/`<TooltipTrigger>`/`<TooltipContent>`; иначе рендерится как раньше без обёртки. `DashboardContent.vue` и `pages/index.vue` — блоки KPI-карточек обёрнуты в `<TooltipProvider>`, `:tooltip` проброшен на все 7 карточек. 318/318 тестов зелёные, `vue-tsc --noEmit` exit 0, locale-sync diff пустой.
- **Settings dialog redesign — sidebar навигация + inline-фидбек** (`beads-task-issue-tracker-fq7`): диалог настроек разросся до 6 секций (Theme, Language, CLI Client, Display, Status Colors, Probe) и уже не помещался в `sm:max-w-lg` без длинного скролла. При попытке переключиться с `bd` на `br` ошибка `switchResult` рендерилась в самом конце диалога — пользователь её не видел и думал, что переключение «не сработало». Это нарушало `.claude/rules/ui-constraints.md` («MUST show errors next to where the action happens»). `SettingsDialog.vue` переписан под layout `sm:max-w-3xl` с `grid grid-cols-[180px_1fr]`: слева — sidebar-навигация (lucide-иконки + лейблы + `aria-current="page"` + `focus-visible:ring`), справа — контент активного раздела. Каждая из 6 секций вынесена в отдельный компонент в `app/components/layout/settings/`: `SettingsAppearance.vue` (Theme + Language вместе — обе про внешний вид), `SettingsCliClient.vue` (с `switchResult` и `isSwitching` спиннером прямо под кнопками `bd`/`br`, `role="status" aria-live="polite"`), `SettingsDisplay.vue`, `SettingsStatusColors.vue` (внутренний скролл секции — основная сетка не растягивается выше `max-h-[80dvh]`), `SettingsProbe.vue` (dev-only). Активный раздел сохраняется в `useLocalStorage('beads:settingsTab')`. Composables (`useTheme`/`useLocale`/`useStatuses`/`useStatusColorOverrides`/`useIssues`/`useCliClient`) не тронуты. RAMS-фиксы той же итерации: probe URL получил `<label class="sr-only">`, probe toggle стал `role="switch" aria-checked` (вместо `aria-pressed`), карточки тем и `bd`/`br` получили явный `focus-visible:ring`. Новый i18n-блок `settings.sections.{appearance,cli,display,colors,probe,navigationLabel}` + `settings.probe.urlLabel`. Доп. dev-only хук `window.__openSettings` в `app.vue` (под `import.meta.dev`) — для будущей UI-проверки через Tauri MCP. 318/318 тестов; `vue-tsc --noEmit` exit 0; визуальная проверка через Tauri MCP — все 5 секций отрисовываются, переключение по sidebar работает, при попытке `bd` → `br` сообщение «'br' not found» появляется прямо под кнопками.

### Fixed
- **Ускорение переключения проектов — устранён дубликат `bd ready`** (`beads-task-issue-tracker-8ow`, `-9oc`): при смене проекта `handlePathChange` в `app/pages/index.vue:523-525` делал два последовательных cold-start'а `bd`: сначала `fetchIssues()` через `bdPollData` (batched `bd list --all` + `bd ready`), затем `fetchStats(issues.value)` снова дергал `bdReady` в `useDashboard.ts:49` (prefetchedReady не пробрасывался). Для Dolt-проектов каждый cold-start = запуск embedded `dolt sql-server` (~2–5 сек), итого −1 лишний запуск сервера на каждое переключение. Fix: переведён на существующий паттерн `fetchPollData()` + `updateFromPollData(issues.value, readyData)` (ровно тот же, что уже используется в polling, `pages/index.vue:232-236`). Параллельно добавлены `performance.now()` метки для всех четырёх фаз (pre-flight / fetchIssues / fetchStats / total) — логируются через `logFrontend('info', '[perf:handlePathChange] ...')` в `beads.log`, чтобы объективно замерять улучшения от будущих оптимизаций (SWR disk-cache `-9id` остаётся открытой задачей). 318/318 тестов зелёные, `vue-tsc --noEmit` exit 0.
- **Workflow KPI теперь учитывает blocked задачи** (`beads-task-issue-tracker-epd`): счётчик `Workflow` на дашборде показывал меньше задач, чем применяемый при клике фильтр. Причина — несогласованность: `isIssueWorkflow()` в `app/utils/issue-helpers.ts` явно исключал blocked через `!isIssueBlocked(issue)`, а `computeWorkflowStatuses` (используется для фильтра) включал blocked, потому что `blocked` относится к категории `wip`, а `WORKFLOW_CATEGORIES = {active, wip, frozen}`. В общепринятой трактовке (Jira и др.) workflow = всё кроме done, блокированные задачи — часть потока работы. Fix: `isIssueWorkflow()` теперь возвращает `true` для всех статусов, кроме `closed`/`deleted`/`tombstone`; `stats.blocked` / `stats.open` / `stats.inProgress` остаются как раньше (блок `if (isIssueBlocked) … else switch(status)` корректно разносит не-workflow-bucket'ы). 3 теста в `tests/utils/dashboard-stats.test.ts` + 1 в `tests/utils/issue-helpers.test.ts` обновлены под новое поведение (workflow=5 вместо 3 в кейсе с 8 issues, default filter view включает blocked). 318/318 тестов зелёные, `vue-tsc --noEmit` exit 0.
