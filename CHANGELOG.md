...
- **Tauri MCP server binary name in `.mcp.json`**: The npm package `tauri-plugin-mcp-server` installs its binary as `tauri-mcp-server` (no `-plugin-` segment). `.mcp.json` referenced the wrong command, so Claude Code never spawned the MCP server and the `mcp__tauri__*` tools were unavailable — visual QA of the Tauri UI silently fell back to "ask the human to look at the screen". `.mcp.json` now uses the correct binary name and the "AI-Driven UI Testing" section in `CLAUDE.md` documents the package/binary name mismatch
