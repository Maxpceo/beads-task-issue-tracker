...
- **Workflow-фильтр теперь показывает все незавершённые статусы, включая кастомные и blocked** (`ud2`): список derived из bd-категорий (`active`/`wip`/`frozen`) через `useStatuses()` вместо захардкоженных 5 встроенных. Review-chain (`inreview`, `simplified`, `reviewed`, `accepted`), `blocked` и любой пользовательский `wip`-статус попадают под Workflow автоматически. Попутно исправлены категории `blocked`/`pinned`/`hooked` в `BUILTIN_FALLBACK` (cold-start до загрузки bd-статусов). **Upgrade note:** если после обновления Workflow-фильтр кажется неполным — кликните KPI «Workflow» ещё раз, чтобы перезагрузить список с учётом кастомных статусов вашего проекта.
...
- **Reference `.claude/references/bd-knowledge.md`** (`beads-task-issue-tracker-ae8`): новая lazy-loaded БЗ по внутреннему устройству bd — источники правды для статусов (`bd statuses` vs `.beads/issues.jsonl` как антипаттерн), таблица категорий с counter-intuitive built-ins (`blocked=wip`, `pinned=frozen`, `hooked=wip`), формат `status.custom` в Dolt (не в `config.yaml`), differences между определениями и использованиями, edge-cases (`BUILTIN_FALLBACK` guard, `bd statuses` vs `bd status`). Файл не автозагружается, читается явно по citation из root `CLAUDE.md` — zero cost токенов до обращения. Первая партия инсайтов — из research над `ud2`.
