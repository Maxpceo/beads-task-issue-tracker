...
- **Faster cold open on large Dolt projects: eliminated `bd ready` subprocess spawn + reliable in-process memo** (`beads-task-issue-tracker-ho6`, `beads-task-issue-tracker-aef`): `bd ready` used to launch a separate process (~700ms on a 1040-issue Dolt project) on every poll cycle. The backend now computes readiness in-process via a pure `compute_ready_from()` function — each poll drops from 2 subprocess spawns to 1 (cold poll ~30% faster on a 1040-issue Dolt project, ≈2.4s → ≈1.7s). An in-process mtime-gated memo layer (`POLL_MEMO`) uses `.beads/interactions.jsonl` as the invalidation signal — bd's append-only field-change audit log (status/priority/label mutations), stable during Dolt sql-server idle activity and bd read operations (`bd list`, `bd show`), bumped on mutations (`bd update --priority`, `bd close`). Initial implementation used `.beads/last-touched` which proved unsuitable: it is bumped on `bd show` (read — false positive) but not on `bd close` on some bd versions (false negative). Pivoted to `interactions.jsonl` after empirical investigation (2026-04-24). Measured on a 1040-issue Dolt project: 28/28 poll cycles stable (100%) over a 10-minute continuous idle window; `bd update --priority` and `bd close` correctly trigger cache invalidation (gate=interactions, mtime changed). Gate cost ≤3ms (`bd_check_changed`). Known gap: `bd create` does not append to `interactions.jsonl` on bd 1.0.2 (only status/field-change entries are logged); 60s TTL bounds staleness for this edge case. Non-Dolt and older bd/br projects retain the original mtime walk via fallback path without regression.
