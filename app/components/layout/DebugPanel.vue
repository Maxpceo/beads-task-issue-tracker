...
import { readLogs, clearLogs, exportLogs as exportLogsApi, getLogPath, getBdVersion, getLoggingEnabled, setLoggingEnabled, getVerboseLogging, setVerboseLogging, checkBdCliUpdate, projectUsesDolt, logFrontend, type BdCliUpdateInfo } from '~/utils/bd-api'
...
watch(() => props.isOpen, async (isOpen) => {
  if (isOpen) {
    ...
    if (beadsPath.value && beadsPath.value !== '.') {
      projectUsesDolt(beadsPath.value)
        .then((uses) => { doltAvailable.value = uses })
        .catch(() => { doltAvailable.value = false })
    } else {
      doltAvailable.value = false
    }
