...
- **Complete-when-ready (strict only):** ready-UI opens only after explicit `plan_mode_complete({ plan })`, **not** from `agent_end`. **Execute path (TUI):** document-flow `ctx.ui.custom(createReadyUiFactory(plan))` **without** `overlay: true` and **without** `SelectList` (live HA 2026-09-18: `SelectList.render` inside custom killed Pi / `TUI.stop()`; questionnaire-style hand-rolled 1–4 / ↑↓ / Enter lives) — 4–6 wrapped-line plan window first (PgUp/PgDn; no j/k dual-focus), then four action labels at the bottom (no descriptions, no Preview). Clean re-show in that loop uses the same custom widget. **Leftover `agent_settled`:** built-in `ctx.ui.select` (custom on settled still aborted live TUI after gauq/m6ho). RPC / missing `custom` uses `select`. If execute `custom` throws, Safe loop degrades (notify + clear pending) — **no** select fallback. Clarifying turns without complete do **not** show Execute.
- **Human transcript before widgets (f3zr):** Pi still writes the full plan or questionnaire prompt+options into the chat via `sendMessage({ display: true }, { triggerTurn: false })` (`plan-ready-document` / `plan-questionnaire`). That `sendMessage` is **not** visibility proof while a blocking widget is open (Pi steers it until the tool returns; select also steals focus). Execute-path readability is the document-flow plan window, not call order. Plan-review dirty findings also go to the transcript (`plan-review-findings`); clean gate adds a short `plan-review-clean` note. During tool execute, transcript `sendMessage` always uses `triggerTurn: false` (51l5). A throw from `sendMessage` must not block the widget.
- Ready actions: execute-path TUI uses document-flow custom **without** `SelectList` (labels Исполнить / Остаться / Уточнить / Отправить на plan-review, no `overlay: true`); leftover/RPC uses `ctx.ui.select` with the same labels. Questionnaire stays document-flow `ctx.ui.custom` (no floating `overlay: true`, no `SelectList`) with digits 1–9 + option preview; RPC/`!hasUI` falls back to capped `select`/`input`.
- Ready button «Отправить на plan-review» runs the same uncapped critique path as `/plan-review`: notify before spawn, keeps plan mode ON, does **not** write `PLAN APPROVED`, does **not** increment `workflow_plan_review` cycle. Dirty (important/critical or gate fail): findings go to the human transcript (`plan-review-findings`, `triggerTurn: false` during execute) **and** return in the `plan_mode_complete` tool result (adjudicate + call complete again); pending cleared; no second widget. Clean: notify + short transcript note + re-show execute-path custom (plan window again) so Maxim can execute. Leftover `agent_settled` dirty path uses a single `sendMessage` with `triggerTurn: true` (no active tool result channel) and leftover select.
...
- `plan_mode_complete({ plan })` — mark the draft plan ready; whitespace-only plan errors. In strict+TUI, opens document-flow `ctx.ui.custom` immediately in the tool execute (not overlay, not `agent_end`). Auto/autopilot notes the call but skips ready-UI and clears pending. If the ready-UI loop throws, notify + clear `pendingReadyPlan` and keep plan mode ON; leftover pending re-opens **select** on the next `agent_settled` (not custom).
...
3. Ready-UI from `plan_mode_complete` execute (same timing as `questionnaire`) is document-flow `ctx.ui.custom` **without** `overlay: true` and **without** `SelectList`. Do **not** open custom ready-UI from `agent_end` / leftover `agent_settled` (gauq, m6ho killed live sessions). `agent_settled` only restores leftover pending via `select`. Transcript `sendMessage` still runs with `triggerTurn: false` but is not proof the plan is readable while the widget is open.
4. Human chooses: execute (durable PLAN APPROVED) / stay / refine / plan-review critique.
5. Plan-review from the button is critique, not approval and not supervisor start. Dirty findings land in the human transcript **and** the `plan_mode_complete` tool result (or leftover single `sendMessage` + `triggerTurn: true` after restart); clean gate notes «чисто» in the transcript and re-shows the execute-path custom widget (plan window again). Leftover residual UX stays select.
