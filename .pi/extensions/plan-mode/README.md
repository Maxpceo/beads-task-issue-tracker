...
- Ready button «Отправить на plan-review» runs the same uncapped critique path as `/plan-review`: notify before spawn, keeps plan mode ON, does **not** write `PLAN APPROVED`, does **not** increment `workflow_plan_review` cycle. Order in the human transcript is always reviewers, then orchestrator adjudication, then the plan. Empty clean (gate ok, no live finding, not BLOCKED, no missing reviewer; `minor: none` and `findings: []` count as empty): `plan-review-human` includes each verdict and «Замечаний нет» plus «Применять нечего, план не менялся»; banner on the re-shown plan is «Plan-review: замечаний к применению нет, план не менялся»; toast may remain; `discussionOpen` stays false; no confirm phrase. Live finding (including one minor) or gate not ok: facts only, pending cleared, `discussionOpen` stays false, no second widget. Tool result tells the orchestrator to call `record_plan_review_adjudication` immediately and not to ask for «покажи план». That tool writes «принято» or «отклонено» plus why, sets the success flag only after the chat write, stamps «План изменён по замечаниям plan-review» only when the text actually changed (banner and trailing whitespace ignored; adjudication prose does not override the diff), and opens ready-UI. Incomplete adjudications or a failed write do not open the plan and do not set `discussionOpen`. `plan_mode_complete` while findings are hanging does not open ready-UI. After the flag, complete does not require a confirm phrase and does not hide the recorded block. BLOCKED or a missing reviewer never says «Замечаний нет» and never clean-reshows. Leftover `agent_settled` writes the same human block with `triggerTurn: true` (no tool-result channel) and does not set `discussionOpen`. RPC/`!hasUI` writes via `sendMessage` when the channel exists and must not claim the overlay is open.
...
5. Plan-review from the button is critique, not approval and not supervisor start. Human order: reviewer facts (`plan-review-human`), orchestrator adjudication (`record_plan_review_adjudication` / `plan-review-adjudication`), then the plan with an honest banner. Empty clean re-shows the execute-path overlay without a confirm phrase. A live finding does not re-show the plan until the adjudication tool succeeds. Toast is optional and never the only proof. Leftover residual UX stays select.
