...
2. First complete after entering plan mode opens ready-UI. After ask/refine, wait for a whole-message confirm before calling `plan_mode_complete({ plan })` again. After dirty plan-review, call `record_plan_review_adjudication` — do not wait for «покажи план» and do not call complete until that tool has written the adjudication. A chat question while ready-UI is open closes the widget before the answer; that dismiss is not Execute, stay, or plan-review and sets `discussionOpen`. The answer turn must not call `plan_mode_complete` and must not wait for a button. Execute before the answer is visible does not write `PLAN APPROVED` and does not dispatch.
