...
- **Complete-when-ready (strict only):** ready-UI opens only after explicit `plan_mode_complete({ plan })`, **not** from `agent_end`. **Execute path (TUI):** document-flow `ctx.ui.custom(createReadyUiFactory(plan))` **without** `overlay: true` — four action labels first, then a 4–6 wrapped-line plan window (PgUp/PgDn; no j/k dual-focus). Clean re-show in that loop uses the same custom widget. **Leftover `agent_settled`:** built-in `ctx.ui.select` (custom on settled still aborted live TUI after gauq/m6ho). RPC / missing `custom` uses `select`. If execute `custom` throws, Safe loop degrades (notify + clear pending) — **no** select fallback. Clarifying turns without complete do **not** show Execute.
...
- Ready actions: execute-path TUI uses document-flow custom (labels Исполнить / Остаться / Уточнить / Отправить на plan-review, no `overlay: true`); leftover/RPC uses `ctx.ui.select` with the same labels. Questionnaire stays document-flow `ctx.ui.custom` (no floating `overlay: true`) with digits 1–9 + option preview; RPC/`!hasUI` falls back to capped `select`/`input`.
...
3. Ready-UI from `plan_mode_complete` execute (same timing as `questionnaire`) is document-flow `ctx.ui.custom` **without** `overlay: true`. Do **not** open custom ready-UI from `agent_end` / leftover `agent_settled` (gauq, m6ho killed live sessions). `agent_settled` only restores leftover pending via `select`. Transcript `sendMessage` still runs with `triggerTurn: false` but is not proof the plan is readable while the widget is open.
