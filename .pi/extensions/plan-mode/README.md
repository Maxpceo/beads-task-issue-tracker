...
- **Human transcript before widgets (f3zr / ph4p):** the full plan is written into the chat immediately via `pi.appendEntry("plan-ready-document")` + `pi.registerEntryRenderer` so mouse-wheel chat scroll works while the overlay is open. `sendMessage` is **not** the plan visibility path (Pi steers it until the tool returns). Questionnaire prompt+options still use `sendMessage({ display: true }, { triggerTurn: false })` (`plan-questionnaire`). Plan-review dirty findings also go to the transcript (`plan-review-findings`); clean gate adds a short `plan-review-clean` note. During tool execute, transcript `sendMessage` always uses `triggerTurn: false` (51l5). A throw from `appendEntry` / `sendMessage` must not block the widget.
...
3. Ready-UI from `plan_mode_complete` execute (same timing as `questionnaire`) is a short `ctx.ui.custom` **with** `overlay: true` (bottom-center, buttons only) and **without** `SelectList`. The full plan is already in the chat via `appendEntry` + entry renderer (mouse wheel scrolls the transcript). Do **not** open custom ready-UI from `agent_end` / leftover `agent_settled` (gauq, m6ho killed live sessions). `agent_settled` only restores leftover pending via `select`. `sendMessage` is not proof the plan is readable while a widget is open.
