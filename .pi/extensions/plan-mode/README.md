...
- **Human transcript before widgets (f3zr):** before blocking ready select / questionnaire UI, Pi puts the full plan or questionnaire prompt+options into the chat via `sendMessage({ display: true }, { triggerTurn: false })` (`plan-ready-document` / `plan-questionnaire`). Plan-review dirty findings also go to the transcript (`plan-review-findings`); clean gate adds a short `plan-review-clean` note. During tool execute, transcript `sendMessage` always uses `triggerTurn: false` (51l5). A throw from `sendMessage` must not block the widget.
...
- `plan_mode_complete({ plan })` — mark the draft plan ready; whitespace-only plan errors. In strict+hasUI, opens `ctx.ui.select` immediately in the tool execute (not custom, not `agent_end`). Auto/autopilot notes the call but skips ready-UI and clears pending. If the ready-UI loop throws, notify + clear `pendingReadyPlan` and keep plan mode ON; leftover pending re-opens select on the next `agent_settled`.
...
3. Ready-UI is `ctx.ui.select` from `plan_mode_complete` execute (same timing as `questionnaire`). Do **not** open `ctx.ui.custom` for ready-UI (`agent_end` / `agent_settled` both killed live sessions: gauq, m6ho). `agent_settled` only restores leftover pending via select. Before select/questionnaire, the full plan or question text is written to the human transcript with `triggerTurn: false`.
4. Human chooses: execute (durable PLAN APPROVED) / stay / refine / plan-review critique.
5. Plan-review from the button is critique, not approval and not supervisor start. Dirty findings land in the human transcript **and** the `plan_mode_complete` tool result (or leftover single `sendMessage` + `triggerTurn: true` after restart); clean gate notes «чисто» in the transcript and re-shows select (plan document again).
