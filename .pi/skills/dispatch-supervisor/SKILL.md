...
6. After supervisor returns, the wrapper inspects status/report, records `DISPATCH RESULT`, and owns the review-transition routing. A complete supervisor artifact (`Status: DONE`/`DONE_WITH_CONCERNS`, `Artifact status: complete`, fresh verification, `Commit` SHA evidence, and a changed `END_COMMIT`) is sufficient for the wrapper to record `WORKFLOW SUBMIT FOR REVIEW` evidence and move the bead to `inreview` without asking the child to call `workflow_submit_for_review`.
