...
2. Update session context:
   ```text
   /workflow-update bead=<ID> session=reviewing
   ```
...
12. Update state after terminal close:
    ```text
    /workflow-update session=closed
    ```
