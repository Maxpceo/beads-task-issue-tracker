...
Do not launch `dispatch_reviewer` or `review_bead` when `.pi/config/workflow-chains.json` `reviewRequired` is the exact boolean `false`. Missing file, missing field, non-boolean, or broken JSON keep review required. Exact `reviewRequired` false does not skip ACCEPTANCE MATRIX when the bead has acceptance/verification bullets. Do not require an `ACCEPTANCE MATRIX:` comment before close only when `matrixRequired` is the exact boolean `false`. Missing file, missing field, non-boolean, or broken JSON keep the matrix required. Exact `matrixRequired` false does not skip `dispatch_reviewer` and does not cancel `reviewRequired`.
...
3. Interactive hop after supervisor complete / `inreview`: one visible code-reviewer pane, not three, not headless `review_bead`:
   ...
   - `APPROVED` → continue acceptance/close below. `NOT APPROVED` → keep `inreview`; `complete_visible_dispatch` does not spawn a supervisor (5o03 owns supervisor-pane reuse via `followup_visible_dispatch({ beadId, role: "<supervisor-role>", task })`). Do **not** call `close_visible_dispatch` on NOT APPROVED / pending-fix — keep the pane for `followup_visible_dispatch`.
