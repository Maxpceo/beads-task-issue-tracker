...
6. Read `.pi/config/workflow-chains.json` from the project (walk from cwd up to the git root; do **not** read `.pi/workflow-chains.json`). If `copyRequired` is false, do **not** run `bd worktree create` or `scripts/setup-worktree.sh`; keep the claimed checkout and continue to step 7. If the file is missing or unreadable, keep the tracker ritual (copy required). Otherwise unconditional create/bind before workflow_plan_mode for every claimed bead that will reach `workflow_plan_approved`, including docs/chore/ci (not only bug/feature/task). Unconditional means every issue type with no user-requested gate; it does **not** mean always run `bd worktree create`. Three-outcome bind:
...
- Do not edit task files before plan approval/auto gate. Required pre-plan bind is **not** a task-file edit: when `.pi/config/workflow-chains.json` has `copyRequired` true (or the file is missing/unreadable), `bd worktree create`, `setup-worktree.sh`, and `workflow_update` (worktree/branch/start) before plan mode are mandatory session bind steps. When `copyRequired` is false, skip create/setup.
