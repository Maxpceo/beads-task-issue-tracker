...
Система управления агентами живёт отдельно от трекера задач. Её можно поставить в любой проект, где есть beads: в трекер, в фонд, в пустую папку.

После установки человек ведёт работу так же, как сейчас в трекере: задача, план, исполнитель, проверка, сдача. Отличие только в стеке. Агент при установке смотрит проект или спрашивает, если папка пустая, предлагает настройки и пишет их после «да». Не угадывает.

В проекте появляются свои исполнители и свой ревьюер под этот стек. Ревьюер трекера в другой проект не копируется как готовый. Проверки, язык задач и правило «бумага или код» лежат в настройках проекта, не в коде системы.

Баг системы чинится в одном репозитории и обновляется у всех. Баг самого проекта остаётся в его задачах.

Трекер после этого — один из потребителей, не дом системы.

Дата: 2026-09-18. Bead: `beads-task-issue-tracker-7fz7`. Epic: `beads-task-issue-tracker-z3h0`.

Это живая спека эпика z3h0. Исполнитель дочерней задачи читает этот файл до кода. Своя задача говорит, что сделать сейчас. Спека говорит, чего не ломать в общей картине. Если они спорят, остановиться и спросить Максима.

Сверяться обязаны: pm0h, bikf, sp57, fd1a, jhux, b18h, z6dd, pv7r, ajex.

## Решения после влития карты (2026-09-22)

Новые настройки не лежат в корне `.pi`. В корне остаются `settings.json` и уже влитый `supervisor-routing.json`. Остальное в `.pi/config/`:

- `verification.json` — какие проверки гонять (pm0h)
- `bead-locale.json` — язык задач (bikf)
- `task-classes.json` — бумага или код (sp57)

Эти файлы не склеивать в один. Онбординг заполняет их после «да» человека.

Пустая папка: стек не виден. Спросить, что это будет, и собрать среду с нуля. Не спрашивать про тесты, которых нет (jhux).

Бумага и код — разные дороги. Бумага только с меткой paper и без правок кода. Агент метку сам не ставит. Без метки задача идёт как код: копия и ревью (sp57). В трекере default остаётся код.
...
4. **После install — разведка стека, не готовый vue.** Скан репо (package.json, pyproject и т.п.). Человек не обязан знать тесты и стек.
5. **Готово только когда среда рабочая**, не когда `pi list` зелёный. Doctor говорит, чего не хватает.
6. **bd назван зависимостью явно** (уже: нет bd → hard fail).

Максим 2026-09-22, подтвердил в чате:

- Любой вопрос агента — человеку. Не угадывать тесты, агентов, язык.
- Человек часто сам не знает ответов. Агент смотрит репо, предлагает («вижу pytest, положу такие проверки — да?») и пишет файлы только после да. Не спрашивает «какие у тебя тесты?» с пустого места.
- Онбординг собирает рабочую среду. Если не хватает куска — говорит вслух.
- Исполнитель кода и ревьюер — под стек проекта, не общие заготовки как финал. Нет агента под стек — предложить создать. Нет ревьюера под стек — предложить создать. Ревьюер трекера (Vue) в sharks не подходит: ритуал ревью общий, чеклист — файл проекта.
- Общий implementer / общий code-reviewer — черновик. Doctor не зелёный, пока человек не подтвердил стековых агентов или явно не сказал «пока хватит общего».

Не брать: `.claude/`, хуки Claude, `npx beads-orchestration`, Kanban UI, worktree `.worktrees/bd-…`, правило «оркестратор не пишет design».

Скилл пакета: detect → install → reload если нужно → скан стека → список пробелов человеку → по «да» создать агентов стека, routing, проверки, правила ревью → doctor: что ещё не хватает.
