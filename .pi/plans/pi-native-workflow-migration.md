...
| `wt` | Live `git -C <ctx.cwd> worktree list --porcelain`; optional workflow override shown as `wf:<name>` | The custom footer omits `wt` in the primary/root checkout. When the current Pi process runs in a linked worktree, it shows `wt:<basename>`; absence of `wt` means no linked worktree is active. `wf:<name>` means workflow state explicitly set a worktree override. |
