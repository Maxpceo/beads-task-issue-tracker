...
```
1. If BEAD_ID provided: read `bd show {BEAD_ID}` and follow orchestrator dispatch context; do not claim, close, push, or change orchestrator-only statuses unless explicitly instructed.
...
# 4. After ALL resolved, report the resolved state and exact recommended commit message to the orchestrator.
# Do not commit, push, acquire merge-slot, release merge-slot, or change bead status unless the dispatch prompt explicitly grants that authority.
```

НЕ ставь статусы review chain (`simplified`, `reviewed`, `accepted`), не закрывай бид, не делай push и не управляй merge-slot — это работа orchestrator'а, если dispatch prompt явно не говорит обратное. Твоя работа заканчивается отчётом с resolved files, verification evidence, recommended commit message, and recommended next status.
...
- Iron Law: am I about to write "tests pass" without running them? (Use project discipline from `AGENTS.md` and `.pi/rules/domain.md`.)
...
**Iron Law (Pi project discipline from `AGENTS.md` and `.pi/rules/domain.md`):** the `Syntax` and `Tests` fields must contain the real command and real output. Banned: "pass", "should work", "probably OK".
