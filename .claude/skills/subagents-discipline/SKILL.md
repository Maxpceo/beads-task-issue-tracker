...
**In completion reports, these phrases are banned:** *"should work / probably / seems / looks correct / выглядит корректно / должно работать / вроде проходит / I'll test it later"*.

Every status claim (tests pass, build ok, bug fixed, acceptance met) requires **fresh evidence in the same message**: the command you ran + its actual output + exit code. If you didn't run the command, write the actual state ("tests not run yet") instead of a guess. Celebratory phrases ("Done!", "Готово!", "Отлично!", "Perfect!") are allowed **only after** the evidence, never instead of it.

When you catch yourself thinking any of the banned phrases → stop, run the thing, cite the output.

```
BAD:  Tests: ✅ (should pass)
GOOD: Tests: pnpm test → exit 0, 256 passed / 256 total
```

This rule is referenced by `CLAUDE.md`, supervisor agents, and `code-reviewer.md`. Violating it in a completion report is a hard failure — orchestrator should redispatch.
...
```bash
bd comments add {BEAD_ID} "APPROACH: Used X instead of Y because Z"
```
...
See the **Iron Law: Evidence Before Claims** section at the top of this file. The banned phrases listed there are the primary red flags — when you catch yourself thinking any of them, stop and run the actual command.
