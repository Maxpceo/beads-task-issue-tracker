...
5. Если нужен Максим или create follow-up при живом родителе: отдельный заголовок ровно `## Дальше` (без названия задачи — оно уже в пункте 1). Не писать `## Дальше — <title>`. Create не конец ответа.
   ...
   Рекомендуемый пункт выделить жёлтым: `### Рекомендую — N` и полный текст этого пункта сразу под ним; остальные пункты обычными номерами. Не оставлять три телеграфные строки без рекомендации.
