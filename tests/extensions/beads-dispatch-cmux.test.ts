...
describe('spawn_task_workspace helpers', () => {
  ...
  it('inlines a two-step parent ping only when both identify refs are present', () => {
    ...
    expect(prompt).toContain('В шаг 1 допиши текст из слов merge готов, фактического SHA коммита и номера PR')
    expect(prompt).toContain('в argv этот текст не вшит')
    expect(prompt).toContain('Текст send не должен содержать \\n или \\r')
    expect(prompt).not.toContain('без Enter')
    expect(prompt).not.toContain('реальный коммит и номер PR, не шаблон')
    expect(prompt).not.toMatch(/cmux send --workspace \S+ --surface \S+ -- '/)
