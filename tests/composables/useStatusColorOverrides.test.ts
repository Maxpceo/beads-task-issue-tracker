...
// The nuxtMetaPlugin in vitest.config.ts replaces import.meta.client → true and
// import.meta.server → false in all /app/ files at transform time, so the real
// useProjectStorage composable works with localStorage in the jsdom environment.
...
describe('useStatusColorOverrides (integration)', () => {
  beforeEach(() => {
    ...
    // Replace global localStorage with a fresh in-memory store for each test
    Object.defineProperty(globalThis, 'localStorage', { value: storage, writable: true, configurable: true })
    localStorage.setItem('beads:path', JSON.stringify(PROJECT_PATH))
    // Reset singleton state in useProjectStorage and useStatusColorOverrides
    clearProjectStorageCache()
  })

  it('setOverride/getOverride/removeOverride round-trip via real composable', () => {
  ...
  it('restores existing overrides from localStorage on first use (persistence across reload)', () => {
    localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ open: { from: '#111111' }, closed: { from: '#222222', to: '#333333' } })
    )
    // Re-run cache clear so useProjectStorage picks up the pre-seeded data
    // (store singleton in useStatusColorOverrides was already populated by prior tests)
    clearProjectStorageCache()
